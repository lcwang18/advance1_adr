...
class AdvancedADRNetWrapper(nn.Module):
    ...
    def __init__(self, config=None):
        ...
        self.loss_manager = LossManager(
            # Keep original training behavior by default:
            # L_sim participates in optimization unless user overrides adv_w_sim.
            w_sim=getattr(config, "adv_w_sim", 1.0),
    ...
    def test_forward(self, image_rgb, image_sar, image_rgb_warp, image_sar_warp, gt_tp, gt_disp):
        ...
        gt_reg_mask = (self._stn_affine(ones, self.gt_tp) > 0.5).float()
        pred_aff_mask = self.net.stn.warp_affine(ones, self.theta)
        pred_reg_mask = (self.net.stn.warp_dvf(pred_aff_mask, outputs["dvf"]) > 0.5).float()
