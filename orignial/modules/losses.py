...
class mi_loss(nn.Module):
    ...
    def forward(self, x, y):
        ...
        if self.sample_ratio < 1.:
            ...
            numel_x = np.prod(x.size()[2:])
            numel_y = np.prod(y.size()[2:])
            
            # 确保x和y的空间维度匹配
            if numel_x != numel_y:
                raise ValueError(f"x and y have different spatial dimensions: {x.size()[2:]} vs {y.size()[2:]}")
            
            idx_th = int(self.sample_ratio * numel_x)
            # 确保idx_choice不超出维度范围
            idx_choice = torch.randperm(int(numel_x))[:idx_th]

            x_flat = x.view(x.size()[0], 1, -1)
            y_flat = y.view(y.size()[0], 1, -1)
            
            # 安全检查索引范围
            max_idx = x_flat.size(2)
            if idx_choice.max() >= max_idx:
                idx_choice = idx_choice[idx_choice < max_idx]
                if len(idx_choice) == 0:
                    idx_choice = torch.tensor([0], device=x.device)
