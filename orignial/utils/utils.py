...
def affine(random_numbers, imgs, padding_modes, opt):
    ...
    if opt.dim == 3:
        ...
        # 计算平移参数 - 考虑图像尺寸，转换为归一化坐标[-1, 1]
        # 获取图像尺寸
        h, w = imgs[0].shape[2], imgs[0].shape[3]
        
        # 将像素平移转换为归一化坐标平移
        # 例如：20像素的平移在256x256图像中是20/256*2 = 0.15625
        translation_pixels = random_numbers[3] * opt.translation
        translation = np.array([translation_pixels / w * 2, translation_pixels / h * 2])
    ...
    # 计算扭曲后的四角点 - 使用动态图像尺寸
    four_corners = np.array([[0, 0], [0, h-1], [w-1, h-1], [w-1, 0]], dtype=np.float32).reshape(-1, 1, 2)
    T = np.array([[2 / w, 0, -1],
                  [0, 2 / h, -1],
                  [0, 0, 1]])
